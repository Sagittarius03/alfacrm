...
class NotificationManager:
    ...
    def send_notification(self, title, message, timeout=10):
        ...
        try:
            notification.notify(
                title=title[:64],  # Максимальная длина для Windows
                message=message[:256],
                timeout=timeout,
    ...
    def notify_lesson_changes(self, changes):
        """Уведомление об изменениях в уроках"""
        if not changes:
            return
    ...
    def notify_schedule_summary(self, lessons):
        """Уведомление с краткой сводкой расписания"""
        ...
        message += f"Свободных слотов: {len(free_slots)}"
